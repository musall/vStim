function vStim_DriftingBar_DualScreen(handles)
...
for iTrials = 1:str2double(handles.NrTrials.String)
    ...
    [StimData.TimeStamps{iTrials},StimData.barShift{iTrials},StimData.FrameCounts{iTrials}] = RunTrial(iTrials); %run current trial cycle
        ...
        sca;
        error('Error during stimulus presentation.')
    end
    ...
    fPath = ['/mnt/smusall/WidefieldImager/Animals/' handles.SubjectName.String '/PhaseMap/']; %path to server
    try
    if IsLinux
        if isdir(fPath)
    ...
    catch disp('Error when trying to save to server');
    end
end
...
function [timeStamps,barShift,FrameCounts] = RunTrial(cTrial)
    ...
    if BasicVarVals(ismember(BasicVarNames,'BarOrient'),cTrial) == 0 %check current bar orientation
        barInd = [1 3];
        screenWidth = screenXpixels/2 + BarWidth;
    else
        barInd = [2 4];
    ...
    while(cTime < absStimTime)
        ...
        if BasicVarVals(ismember(BasicVarNames,'BarOrient'),cTrial) == 0 %check current bar orientation
            temp = false(1,4);
            temp(barInd) = destRect(barInd) > screenXpixels / 2;
            destRect(temp) = screenXpixels / 2;
        end
        ...
        Screen('DrawTexture', window, cTex,srcRect,destRect,0); %draw texture
        
        %draw second texture for vertical orientation
        if BasicVarVals(ismember(BasicVarNames,'BarOrient'),cTrial) == 0 %check current bar orientation
            if BasicVarVals(ismember(BasicVarNames,'BarDirection'),cTrial) == 0
                temp = screenXpixels - [barShift(tCnt) barShift(tCnt)-BarWidth]; %change bar destination coordinates
                temp(temp < screenXpixels / 2) = screenXpixels/2; %don't cross to the other side of the screen
                destRect(barInd) = temp; %change bar destination coordinates
            else
                temp = screenXpixels - [barShift(end-tCnt+1) barShift(end-tCnt+1)-BarWidth]; %change bar destination coordinates
                temp(temp < screenXpixels / 2) = screenXpixels/2; %don't cross to the other side of the screen
                destRect(barInd) = temp; %change bar destination coordinates
            end
        end
